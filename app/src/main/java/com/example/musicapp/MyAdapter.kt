...
class MyAdapter(val context: Activity, val dataList:List<Data>): RecyclerView.Adapter<MyAdapter.MyViewHolder>() {
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        holder.title.text=currentData.title // showing the title of the music

        Picasso.get().load(currentData.album.cover).into(holder.image)     //showing the music cover image
    ...
    class MyViewHolder(itemView: View): RecyclerView.ViewHolder(itemView){
        ...
        init {
            image = itemView.findViewById<ImageView>(R.id.musicImage)
            title = itemView.findViewById<TextView>(R.id.musicTitle)
